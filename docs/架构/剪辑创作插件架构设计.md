...
Catalog 启动时校验全部固定内容：rule 必须包含非空内容，skill frontmatter 必须是合法
YAML mapping，且包含与资源 URI 一致的名称和非空说明，schema 必须是 JSON object。
...
`common.schema.json` 的根合同接受 `ArtifactRef`、`TimeRangeUs`、`SuccessResponse`
