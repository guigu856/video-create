...
汇总分析提交确认前同时生成三个阶段投影，并由 `reference_report_manifest.json` 列出对应文件：
...
该文件固定列出总体报告、BGM 分析、逐镜分析、剪辑规律、阶段投影和 EvidenceBundle
的相对路径与 SHA-256。报告工具从已校验的分析输出生成报告和清单；用户确认后，
Agent 才发布其中的可迁移知识。
