...
内部逐镜分析继续按 `RhythmUnit → MainShot → InShotEvent` 组织，前五章正文翻译为主要画面、画面里的变化、声音和画面的配合及可以重复使用的剪法。第三章使用连续全片时间线和重点片段详解，不承担新素材的逐镜执行设计。重要事实和判断标明证据与把握程度；tempo、beat、候选边界等算法结果保留候选身份。
