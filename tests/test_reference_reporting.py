...
def test_generate_report_writes_json_markdown_and_hash_manifest(tmp_path: Path) -> None:
...
def test_generated_report_is_openable_and_byte_stable(tmp_path: Path) -> None:
    ...
    overview = tmp_path / second.manifest.file_for("video_overview").path
    shots = tmp_path / second.manifest.file_for("shot_analysis_markdown").path
    assert overview.read_text(encoding="utf-8").startswith("# 参考视频总体理解")
    assert "| 镜头 | 原片时间" in shots.read_text(encoding="utf-8")
